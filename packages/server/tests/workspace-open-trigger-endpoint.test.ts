...
beforeEach(() => {
  repo = makeRepoFixture("clobber-workspace-open-");
});
...
describe("POST /workspaces/:id/open — fire workspace-open trigger", () => {
  it("fires registered workspace-open trigger and returns dispatched count", async () => {
    ...
    const patchRes = await h.server.inject({
      ...
      payload: { triggers: [{ kind: "workspace-open" }] },
    });
  ...
  it("returns dispatched=0 when no agent in the workspace has a workspace-open trigger", async () => {
    const h = buildHarness();
    const boot = await bootInWorkspace(h, repo.path);
  ...
  it("returns 404 for an unknown workspace id", async () => {
    ...
    const fireRes = await h.server.inject({
      method: "POST",
      url: "/workspaces/00000000-0000-0000-0000-000000000000/open",
      payload: {},
    });
    expect(fireRes.statusCode).toBe(404);

    await teardown(h);
  });

  it("debounces a rapid second call within the default window", async () => {
    const h = buildHarness();
    const boot = await bootInWorkspace(h, repo.path);
    ...
    expect((first.json() as { dispatched: number }).dispatched).toBe(1);
    ...
    expect((second.json() as { dispatched: number }).dispatched).toBe(0);
