...
// task kick. Layer-B content now arrives via a seed (#211) rather than the
// retired workspace boot-context provider.
describe("system-prompt composition (A/B/C)", () => {
  it("composes layer-A framing + layer-B seed + office continuity into the system prompt; the user turn is just the task kick", async () => {
    const h = buildHarness(turnProvider());
    const seedDir = join(h.repoPath, ".clobber", "seeds", "framing-probe");
    ...
    refSeed(h, role.id, "framing-probe");
...
// Make the role reference one seed by name (replacing its default refs) so the
// composition under test is isolated to that seed.
function refSeed(h: Harness, roleId: string, name: string): void {
