...
const CYCLE_USAGE = `usage: clobber cycle [--agent <agent>] --prompt <text>
...
export const cycleCommand: Command = {
  ...
  usage: CYCLE_USAGE,
  async run(ctx) {
    const { agent, prompt, token } = parseCycleArgs(ctx.args);
    ...
    if (token !== undefined) {
      if (agent !== undefined || prompt !== undefined) {
        throw new CliUsageError("--token redeems a saved cycle; do not combine it with --agent/--prompt");
      }
